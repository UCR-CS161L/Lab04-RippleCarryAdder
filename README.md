...
you send that value to the next highest order digit. Then procede until the highest non-zero digit is reached. While in electronics everything is done in parallel, the actual result from the lowest 
order bit takes some time, know as **propagation delay**, to settle on the actual result before 
sending the carry value to the next digit. The next digit cannot settle on it final result until
...
For the prelab you will turn in your test-bench file only. You should right this prelab before 
...
in the prelab to Gradescope all the tests will fail. That is expected. Your grade for the prelab
...
as part of this lab GitHub repository in ripple_carry_adder_tb.v.
...
actually equals A+B, and then also test the boundaries where carry out should occur, such as 127 + 1. Then add a couple of random values, just to be sure.
...
create increasing larger adders, doubling the number of bits each time, until in the given time, 
...
the size you think will fail. Hint: it won't take long.
...
adding A and B, and a carry out, which indicates what the carry out of the highest order bit of the
addition operation. The only file you need to write code in, other than the test-bench, is 
[riplle_carry_adder.v](./ripple_carry_adder.v). Do not modify any of the other files. 

Hint: Your ripple carry adder will instatiate and wire up N full_adder modules.
